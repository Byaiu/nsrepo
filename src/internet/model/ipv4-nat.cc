  ...
  if (m_ipv4->GetInterfaceForDevice (in) == m_outsideInterface)
    {
      ...
      NS_LOG_DEBUG ("destination address -> " << destAddress);
      //Checking for Static NAT Rules
      for (StaticNatRules::const_iterator i = m_statictable.begin ();
           i != m_statictable.end (); i++)
        {
          NS_LOG_DEBUG ("IP Header protocol -> " << static_cast<int> (ipHeader.GetProtocol()) << " Rule protocol -> " << (*i).GetProtocol());
          ...
          else
            {
              ...
              if (ipHeader.GetProtocol () == IPPROTO_TCP
                           ...
                           || (*i).GetProtocol () == 0))
                {
                  ...
                  NS_LOG_DEBUG ("UDP destination port -> " << udpHeader.GetDestinationPort());
  ...
  if (m_ipv4->GetInterfaceForDevice (out) == m_outsideInterface)
    {
      ...
      NS_LOG_DEBUG ("source address -> " << srcAddress);
