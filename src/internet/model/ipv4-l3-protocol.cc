          ...
          if (sendIt)
            {
              ...
              if (m_netfilter != 0)
                {
                  NS_LOG_INFO ("NF_INET_LOCAL_OUT Hook");
                  Verdicts_t verdict = (Verdicts_t) m_netfilter->ProcessHook (PF_INET, NF_INET_LOCAL_OUT, pkt, 0, device);
                  if (verdict == NF_DROP)
                    {
                      NS_LOG_INFO ("NF_INET_LOCAL_OUT packet not accepted");
                      // Add drop trace here
                      return;
                    }
                }
              ...
              if (m_netfilter != 0)
                {
            	  NS_LOG_INFO ("NF_INET_POST_ROUTING Hook");
                  Callback<uint32_t, Ptr<Packet> > ccb = MakeCallback (&Ipv4Netfilter::NetfilterConntrackConfirm, m_netfilter);
                  Verdicts_t verdict = (Verdicts_t) m_netfilter->ProcessHook (PF_INET, NF_INET_POST_ROUTING, pkt, 0, device, ccb);
                  if (verdict == NF_DROP)
                    {
                	  NS_LOG_INFO ("NF_INET_POST_ROUTING packet not accepted");
                      // Add drop trace here
                      return;
                    }
                }
